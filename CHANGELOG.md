...
- **First slice: ingestion (text/web/PDF with local hybrid OCR) + citation-grounded chat, driven
  ...
  **No fetch/network tool is reachable at question-answering time.** Early designs considered
  ...
  external ones. Ingestion-time fetching is host-side and one-shot instead — see invariant 1.
  ...
  **Citation verification is coordinate-only, and says so.** `citations.py` confirms a `Citation`'s
  ...
  prose is faithful to that text, and no docstring or UI copy should imply otherwise (invariant 4).
  ...
  re-run rarely fixes a persistent (rather than transient) coercion failure.
  ...
  **`injection_scan.py` flags, never blocks.** A deterministic heuristic scan runs at ingestion
  ...
  flag surfaced as metadata alongside it (invariant 5) — this is a transparency mechanism, not a
  ...
  **OCR ships enabled, not merely pluggable.** `parsers/pdf.py` uses `pymupdf4llm`'s built-in hybrid
  OCR (RapidOCR primary, Tesseract fallback) for scanned/image PDF pages, with the actual backend
  installed via the `ocr` extra rather than left as an unconfigured interface — a sibling
  open-source project shipped the pluggable-but-off version and silently failed to parse image
  sources in its default Docker image; see invariant 6.
