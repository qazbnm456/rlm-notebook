...
CLAUDE.md invariant 4: this checks coordinate existence ONLY — that `source_id` exists in the
...
def verify_citations(citations: list[Citation], corpus: Corpus) -> list[VerifiedCitation]:
    ...
    is ever dropped (CLAUDE.md invariant 4); a citation that fails is returned with
